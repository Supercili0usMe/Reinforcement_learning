...
def calc_loss(batch, net, tgt_net, gamma, device="cpu", double=True):
    ...
    # Make sure tensors have correct dtypes (float32 for network inputs/targets,
    # long for indices and bool for masks) to avoid dtype-mismatch errors.
    ...
    if double:
        # choose best actions by the online network
        next_state_actions = net(next_states_v).max(1)[1]
        # evaluate those actions by the target network
        next_state_values = tgt_net(next_states_v).gather(1, next_state_actions.unsqueeze(-1)).squeeze(-1)
    else:
        next_state_values = tgt_net(next_states_v).max(1)[0]
    # mask terminal states: use boolean mask (dtype=torch.bool)
...
def calc_values_of_states(states_tensor, net, device="cpu"):
    # states_tensor is expected to be a torch.Tensor with dtype=float32 already on correct device
    # Use no_grad to avoid autograd overhead during evaluation.
    mean_vals = []
    net_was_training = net.training
    net.eval()
    with torch.no_grad():
        for batch in torch.split(states_tensor, 64):
    ...
    with common.RewardTracker(writer, params['stop_reward']) as reward_tracker:
        while True:
            ...
            if frame_idx % EVAL_EVERY_FRAME == 0:
                mean_val = calc_values_of_states(eval_states_tensor, net, device=device)
